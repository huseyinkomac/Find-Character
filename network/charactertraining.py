...
train_images = np.array([i[0] for i in train_data]).reshape(-1, 50, 50, 1)
train_labels = [i[1] for i in train_data]
train_labels = keras.utils.to_categorical(train_labels, 62)
test_images = np.array([i[0] for i in test_data]).reshape(-1, 50, 50, 1)
...
def train_cnn(model):
    ...
    model.fit(train_images, train_labels, batch_size=20, epochs=4, verbose=1, validation_data=(test_images, test_labels))
